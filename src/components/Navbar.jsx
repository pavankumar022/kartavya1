  ...
  return (
    <nav className="sticky top-0 z-50 bg-white shadow-md border-b border-gray-200">
      <div className="w-full px-4 sm:px-8">
        <div className="flex items-center justify-between h-20 w-full gap-6">
          {/* Logo */}
          {sidebarOpen ? (
          ...
          <div className="hidden md:flex flex-1 items-center justify-center font-nav">
            <div className="flex items-center gap-6 bg-white border border-gray-200 rounded-full px-8 py-2 shadow-sm">
              {navItems.map((item) => (
                <button
                  key={item.id}
                  onClick={() => onNavigate(item.id)}
                  className={`px-3 py-2 rounded-full text-xl font-semibold transition-all ${
                    currentPage === item.id
          ...
          <div className="flex items-center space-x-4 flex-shrink-0">
            <ClickBubbleEffect>
              <button
                ...
                className="px-8 py-3 bg-dark-red text-white rounded-lg font-semibold text-base hover:bg-dark-red-light transition-colors shadow-md"
              >
